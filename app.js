...
(function seedIfEmpty(){
  if(!DB.read('products')){
    const demo = [
      ...
      {sku:'7501258203418', nombre:'SERRALPINA/ HIOSCINA METAMIZOL SODICO', etiqueta:'Marca', precio:190, costo:130, stock:98, min:50, lotes:[{lote:'D1',pzas:98,cad:'2026-11-07'}], img:''},
      {sku:'7501080499702', nombre:'TABCIN NOCHE', etiqueta:'Marca', precio:101, costo:70, stock:99, min:50, lotes:[{lote:'E1',pzas:99,cad:'2026-07-26'}], img:''},
      {sku:'7501300450562', nombre:'ACLARIUM/ AZITROMICINA', etiqueta:'Marca', precio:263, costo:180, stock:99, min:50, lotes:[{lote:'F1',pzas:99,cad:'2026-08-13'}], img:''},
      {sku:'7502216790124', nombre:'INDOMETACINA, BETAMETASONA, METOCARBAMOL', etiqueta:'Genérico', precio:352, costo:250, stock:99, min:50, lotes:[{lote:'G1',pzas:99,cad:'2026-06-01'}], img:''},
      {sku:'7502241010897', nombre:'REUMO FX/ COLAGENO HIDROLIZADO', etiqueta:'Marca', precio:763, costo:520, stock:50, min:50, lotes:[{lote:'H1',pzas:50,cad:'2027-01-01'}], img:''},
      {sku:'7501299307694', nombre:'SELTAFERON 75/ OSELTAMIVIR', etiqueta:'Genérico', precio:534, costo:400, stock:99, min:50, lotes:[{lote:'I1',pzas:99,cad:'2026-07-07'}], img:''},
    ];
    DB.write('products', demo);
  }
...
function renderVentas(){
  ...
  grid.innerHTML = '';
  products.forEach(p=>{
    const sTexto = p.stock==null ? '—' : p.stock;
    ...
    card.innerHTML = `
      ...
      <div class="row">
        <div><strong>${money(p.precio)}</strong> <span class="muted">Stock: ${sTexto}</span></div>
...
function addToCart(sku){
  ...
  if(existing){ existing.cant++; } else { CART.push({sku:p.sku, nombre:p.nombre, precio:p.precio, cant:1}); }
  renderCart();
}
function renderCart(){
  const box = $('#cart');
  ...
  CART.forEach((i,idx)=>{
    ...
    row.innerHTML = `
      ...
      <div class="qty">
        <button data-a="-">−</button><div>${i.cant}</div><button data-a="+">+</button>
      </div>
  ...
  const extraDesc = $('#extraDesc').dataset.savedDesc;
  const extraMonto = Number($('#extraMonto').dataset.savedMonto||0);
  if(extraDesc && extraMonto>0){
    total += extraMonto;
    const r = document.createElement('div');
    r.className='cart-item';
    r.innerHTML = `<div><strong>${extraDesc.toUpperCase()}</strong></div><div></div><div>${money(extraMonto)}</div><button class="btn danger" id="delExtra">✕</button>`;
    box.appendChild(r);
    $('#delExtra').onclick=()=>{ delete $('#extraDesc').dataset.savedDesc; delete $('#extraMonto').dataset.savedMonto; $('#extraDesc').value=''; $('#extraMonto').value=''; renderCart(); }
  }
  $('#total').textContent = money(total);
}
$('#agregarExtra').onclick = ()=>{
  const d = $('#extraDesc').value.trim();
  const m = Number($('#extraMonto').value||0);
  if(!d || m<=0) return alert('Captura descripción y monto válido');
  $('#extraDesc').dataset.savedDesc = d;
  $('#extraMonto').dataset.savedMonto = m;
  renderCart();
};
...
$('#btnCobrar').onclick = ()=>{
  ...
  const ventaItems = CART.map(i=>({sku:i.sku, nombre:i.nombre, precio:i.precio, cant:i.cant}));
  const extra = $('#extraDesc').dataset.savedDesc ? {desc:$('#extraDesc').dataset.savedDesc, monto:Number($('#extraMonto').dataset.savedMonto)} : null;
  const total = ventaItems.reduce((a,b)=>a+b.precio*b.cant,0) + (extra?extra.monto:0);

  // Método de pago
  const metodo = $('#metodoPago').value; // EFE/TAR/TRA/OTR
  const ref = $('#refPago').value.trim();
  ...
  sales.unshift({
    ...
    items:ventaItems, extra, total, pago:metodo, referencia:ref||null, receta:recetaFile, estado:'Activa'
  });
  DB.write('sales',sales);

  // Ticket
  printTicket({folio,total,fecha:new Date(), items:ventaItems, extra, pago:metodo, referencia:ref||null});
...
function printTicket({folio, total, fecha, items, extra, pago, referencia}){
  const lines = [];
  lines.push('   Farmacia DP');
  lines.push('   SOLIDARIDAD');
  lines.push('   5526234556  •  FARMACIASDP@GMAIL.COM');
  lines.push('');
  lines.push(`Folio: ${folio}`);
  lines.push(fecha.toLocaleString('es-MX'));
  lines.push('--------------------------------');
  items.forEach(i=>{
    lines.push(i.nombre.slice(0,28));
    lines.push(` ${i.cant} x ${money(i.precio).padStart(8)}  ${money(i.cant*i.precio).padStart(8)}`);
  });
  if(extra){
    lines.push(extra.desc.slice(0,28));
    lines.push(` 1 x ${money(extra.monto).padStart(8)}  ${money(extra.monto).padStart(8)}`);
  }
  lines.push('--------------------------------');
  lines.push(`Total           ${money(total)}`);
  lines.push(`Pago: ${pago}`);
  if(referencia) lines.push(`Ref: ${referencia}`);
  lines.push('');
  lines.push('¡Gracias por su compra!');
  const w = window.open('','ticket','width=380,height=600');
  w.document.write(`<pre style="font:14px/1.25 monospace; white-space:pre-wrap">${lines.join('\n')}</pre><script>window.print();setTimeout(()=>window.close(),300);</script>`);
  w.document.close();
}
...
function renderInventario(){
  ...
  prods.forEach(p=>{
    ...
    tr.innerHTML = `
      ...
      <td>${p.sku}</td>
      <td>${p.nombre}</td>
      <td><span class="pill">${p.etiqueta}</span></td>
      ...
      <td>${cadprox}</td>
      <td>
        <button class="btn light" data-ed="${p.sku}">Editar</button>
        ${p.stock==null?'':`<button class="btn light" data-add="${p.sku}">+Stock</button>`}
        <button class="btn gray" data-del="${p.sku}">Eliminar</button>
      </td>
    `;
    ...
    if(btnAdd) btnAdd.onclick=()=> modalAddStock(p.sku);
...
function modalEditProd(sku){
    ...
    <div style="position:fixed;inset:0;background:#0006;display:grid;place-items:center;z-index:50">
      <div class="card pad" style="width:min(920px,94vw)">
        ...
        <div class="grid" style="grid-template-columns:1fr 1fr; gap:10px">
          ...
          <div>Stock (vacío para servicio) <input id="fstock" type="number" class="input" value="${p.stock??''}" placeholder="—"></div>
  ...
  wrap.querySelector('#save').onclick=async ()=>{
    const skuV = wrap.querySelector('#fsku').value.trim();
    if(!skuV) return alert('SKU requerido');
    const nom = wrap.querySelector('#fnom').value.trim()||'';
    const etq = wrap.querySelector('#fetq').value;
    const precio = Number(wrap.querySelector('#fpre').value||0);
    const costo = Number(wrap.querySelector('#fcosto').value||0);
    const min = Number(wrap.querySelector('#fmin').value||0);
    const stockVal = wrap.querySelector('#fstock').value;
    const stock = stockVal==='' ? null : Number(stockVal);
    // imagen local (DataURL)
    const file = wrap.querySelector('#fimg').files[0];
    let img = p.img||'';
    if(file) img = await readFileAsDataURL(file);

    const existingIdx = prods.findIndex(x=>x.sku==skuV);
    const obj = {sku:skuV, nombre:nom, etiqueta:etq, precio, costo, stock, min, lotes:p.lotes||[], img};
    if(existingIdx>=0) prods[existingIdx] = obj;
    else prods.push(obj);
    DB.write('products',prods);
    wrap.remove(); renderInventario(); renderVentas();
  };
}
...
function modalAddStock(sku){
  ...
  wrap.querySelector('#ok').onclick=()=>{
    const lote = wrap.querySelector('#slote').value.trim();
    const cad = wrap.querySelector('#scad').value||'';
    const pzas = Number(wrap.querySelector('#spzas').value||0);
    if(!lote || pzas<=0) return alert('Completa lote y piezas');
    p.lotes = p.lotes||[];
    p.lotes.push({lote, cad, pzas});
    p.stock = (p.stock||0)+pzas;
    DB.write('products',prods);
    wrap.remove(); renderInventario();
  };
}
...
function renderHist(){
  ...
  DB.read('sales',[]).forEach(s=>{
    ...
    tr.innerHTML = `
      ...
      <td>${s.receta?'📎':''}</td>
      <td>${s.estado}</td>
      <td><button class="btn light" data-ver="${s.folio}">Ver</button> <button class="btn danger" data-an="${s.folio}">Anular</button></td>
    `;
...
function showSale(folio){
  const s = DB.read('sales',[]).find(x=>x.folio==folio);
  if(!s) return;
  const wrap = document.createElement('div');
  wrap.innerHTML = `
    <div style="position:fixed;inset:0;background:#0006;display:grid;place-items:center;z-index:50">
  ...
  wrap.querySelector('#reimprimir').onclick=()=> printTicket({
    folio:s.folio,total:s.total,fecha:new Date(s.fecha), items:s.items, extra:s.extra, pago:s.pago, referencia:s.referencia
  });
}
function cancelSale(folio){
  const prods = DB.read('products',[]);
  const sales = DB.read('sales',[]);
  const s = sales.find(x=>x.folio==folio);
  if(!s || s.estado!=='Activa') return;
  if(!confirm('¿Anular venta y regresar stock?')) return;
  s.items.forEach(i=>{
    const p = prods.find(x=>x.sku==i.sku);
    if(p && p.stock!=null) p.stock += i.cant;
  });
  s.estado = 'Anulada';
  DB.write('products',prods); DB.write('sales',sales);
  renderHist(); renderInventario(); renderReportes(); renderDashboard();
}
...
function renderReportes(){
  ...
  const prom = sales.length? total/sales.length : 0;
  const sum = {EFE:0,TAR:0,TRA:0,OTR:0};
  sales.forEach(s=>{ sum[s.pago]=(sum[s.pago]||0)+s.total; });
  ...
  const cnt = {};
  sales.forEach(s=>s.items.forEach(i=>{ cnt[i.nombre]=(cnt[i.nombre]||0)+i.cant; }));
...
function renderDashboard(){
  ...
  const total = sales.reduce((a,b)=>a+b.total,0);
  const items = sales.reduce((a,b)=> a + b.items.reduce((x,y)=>x+y.cant,0) ,0);
  $('#kpiVentas').textContent = money(total);
  $('#kpiTickets').textContent = sales.length;
  $('#kpiItems').textContent = items;
  $('#kpiProm').textContent = money(sales.length? total/sales.length : 0);

  const prods = DB.read('products',[]);
  const bajas = prods.filter(p=>p.stock!=null && p.stock<= (p.min||0)).map(p=>`${p.nombre} (${p.stock})`).join(', ');
...
function daysTo(d){ const a = new Date(d); const b = new Date(); return Math.ceil((a-b)/86400000); }
